...
tbl_lower <- tbl_lower %>% 
  mutate(
      predict_keras = predict(fit_keras, mat_lower)
    , residual_keras = incremental_paid - predict_keras
  )

tbl_compare <- tbl_lower %>% 
  group_by(AccidentYear) %>% 
  summarise(
      ibnr = sum(IBNR)
    , ibnr_glm = sum(predict_glm)
    , ibnr_keras = sum(predict_keras)
    , rmse_glm = mean(residual_glm ^ 2) %>% sqrt()
    , rmse_keras = mean(residual_keras ^ 2) %>% sqrt()
  )
```
