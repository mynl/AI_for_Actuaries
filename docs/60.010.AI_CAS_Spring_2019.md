...
* \callout{Predictive} modeling or analytics: estimating value for a \callout{new} unit
...
* \callout{Machine learning}: an algorithm allowing computer to build and improve a model from \callout{data}
...
* **Not** ML: expert system, knowledge base
...
* Previously machine learning research was fragmented with distinct approaches for natural language processing, vision, motion planning and speech recognition
...
* Build specialized problem solving nodes
  - recognize color...
...
* Nodes simultaneously learn model and data representation
...
* Bigger = \callout{deeper} models enabled by greater computational power and better algorithms
...
\tagline{...ML tools work well on real data}
...
* For $p=0.01$ see plenty of examples on r/CatastrophicFailure, r/IdiotsInCars, r/nononono, r/OSHA, r/Roadcam
...
AI is being applied to high impact weather forecasting
...
\node[textbox] (A) at (page cs:0, 0.85) {\Large  ``{\bf Raw data} is both an oxymoron and a bad idea; \\ \Large to the contrary, data should be cooked with care.'' \\ \footnotesize Geoffrey Bowker  };

\node[textbox, shadedbox, below right] (pros) at (page cs:-0.1, -.30) {\large \bf Message so Far \\
\tzsbullet Show don't tell \\
\tzsbullet Input training data with responses \\
...
* Solutions
  1. Enhanced methodological rigor in the collection and management of training data
...
\tagline{...AI is just a machine---think washing machine \\ not HAL9000---powerful but limited \\ \tiny Benedict Evans }
